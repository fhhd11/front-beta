...
## 🧠 Проблемы с контекстом и памятью

### Ошибка автоматической суммаризации
```
ValueError: No assistant message found from indices X to Y
```

**Причина:**
Система пытается суммаризировать контекст, когда в истории слишком мало сообщений (менее 15).

**Решение:**
Фронтенд теперь автоматически проверяет количество сообщений перед попыткой суммаризации:
- Минимальное количество сообщений для суммаризации: **15**
- Автоматическая суммаризация срабатывает при заполнении контекста на **80%+**
- Параметр `max_message_length`: **10** (сколько последних сообщений оставить после суммаризации)

**Как работает защита:**
1. Перед каждой отправкой сообщения проверяется состояние контекста
2. Если сообщений < 15, суммаризация **пропускается**
3. Если контекст заполнен на 80%+ и сообщений достаточно, происходит автоматическая суммаризация
4. Если суммаризация не удалась, сообщение все равно отправляется (graceful degradation)

**Ручная проверка:**
```javascript
// В консоли браузера
console.log('Messages:', contextWindow.memoryStats.messages)
console.log('Context usage:', contextWindow.contextUsage.percentage)
```
